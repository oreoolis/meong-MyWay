...
description: >
  ...
  accent, the three-step gray text ladder, Segoe UI Variable (display/numerals/chrome) + Segoe UI Variable (body),
...
typography:
  # — reading ramp (Segoe UI Variable body + Segoe UI Variable chrome) —
  body:    { fontFamily: "Segoe UI Variable", cqw: 0.85, weight: 400, lineHeight: 1.6, color: "text-muted" }
  h4-eyebrow:{ fontFamily: "Segoe UI Variable", cqw: 0.8, weight: 600, tracking: "0.08em", upper: true, color: "primary" }
  tag:     { fontFamily: "Segoe UI Variable", px: 12, weight: 500, color: "primary" }
  counter: { fontFamily: "Segoe UI Variable", px: 13, weight: 500, tracking: "0.05em", color: "text-muted" }
  # — display / numerical ramp (Segoe UI Variable, near-black headings / cobalt numerals) —
  h3:      { fontFamily: "Segoe UI Variable", cqw: 1.25, weight: 500, lineHeight: 1.3, tracking: "-0.02em", color: "text" }
  stat-num:{ fontFamily: "Segoe UI Variable", cqw: 1.9, weight: 700, lineHeight: 1.0, color: "primary" }
  blockquote:{ fontFamily: "Segoe UI Variable", cqw: 2.4, weight: 500, lineHeight: 1.35, color: "text" }
  h2:      { fontFamily: "Segoe UI Variable", cqw: 2.6, weight: 600, lineHeight: 1.1, tracking: "-0.02em", color: "text" }
  metric-value:{ fontFamily: "Segoe UI Variable", cqw: 3.0, weight: 700, lineHeight: 1.0, color: "primary" }
  h1:      { fontFamily: "Segoe UI Variable", cqw: 4.2, weight: 700, lineHeight: 1.08, tracking: "-0.02em", color: "text" }
  quote-mark:{ fontFamily: "Segoe UI Variable", cqw: 8.0, weight: 700, lineHeight: 0.5, color: "primary", opacity: 0.15 }
...
components:
  ...
  cta-button:
    ...
    textColor: "{colors.bg}"
    rounded: "{radii.pill}"
    typography: "Segoe UI Variable 600"
...
- **Fonts** — already set to **Segoe UI Variable** (display) / **Iowan Old Style** (body); ignore any preset font name lingering in prose.
...
The voice is two faces in fixed roles: **Segoe UI Variable** (display, every numeral, all chrome —
eyebrows uppercase 0.08em) and **Segoe UI Variable** (body, muted gray, line 1.6). Headlines are near-black;
...
- **Segoe UI Variable** (display/numerals/chrome) + **Segoe UI Variable** (body) — near-black headlines, cobalt numerals.
...
Two ramps. The **reading ramp** (Segoe UI Variable body 0.85cqw muted; Segoe UI Variable eyebrow uppercase 0.08em
cobalt) carries copy + chrome; the **display/numerical ramp** (Segoe UI Variable `h3` 1.25cqw → `h1`
...
- **Headlines near-black, −0.02em**; **eyebrows cobalt, uppercase, 0.08em**; **numerals cobalt 600–700**; **body Segoe UI Variable 400 muted, line 1.6**. No italic, no uppercase body, no cobalt headline.
...
**Composes** accent-line, meta, h1, body sub. **Focal** a 2-line Segoe UI Variable `h1` near-black, left,
...
**Focal** a row of tinted cards — cobalt `metric-value` + Segoe UI Variable label + muted desc + optional
...
blockquote, cite. **Focal** a Segoe UI Variable `blockquote` near-black under a 15%-opacity cobalt
...
Segoe UI Variable body column beside a cobalt-8% highlight block (4px cobalt left rule) carrying an inline pull
...
a Segoe UI Variable `h1` near-black, centered, with the one solid cobalt `cta-button` pill below. **Accent**
...
- Use **tinted cards** (4% fill, 20% border, 10–14px radius, no shadow); body Segoe UI Variable 400 muted, line 1.6.
...
- **Type** — Segoe UI Variable headings −0.02em near-black, cobalt eyebrows 0.08em + numerals; Segoe UI Variable body muted line 1.6; ≥1.4cqw floor.
...
- **Segoe UI Variable + Segoe UI Variable via Google Fonts.** CJK pairing (Noto Sans SC 700 display / Noto Serif SC 400 body) carries over; the eyebrow's uppercase+tracking signal weakens in CJK — pair it with the accent-line.

  ...
  try {
    while (true) {
      ...
      while ((boundary = pending.indexOf("\n")) >= 0) {
        ...
        if (frame.type === "phase" && ["parsing", "context", "embedding", "planning", "specialists", "complete"].includes(frame.phase)) onProgress?.(frame.phase, typeof frame.thought === "string" ? frame.thought : undefined, typeof frame.agent === "string" ? frame.agent : undefined);
